...
class Proveedores{
  ...
  public function getList($values)
  {
    ...
    {
      $where.=" AND upper(Zona) like ('%".strtoupper($values['columns'][6]['search']['value'])."%')";
    }
    ...
    ->join("ProveedoresTipos","INNER JOIN ProveedoresTipos pt on pt.IdProveedorTipo = Proveedores.IdProveedorTipo")
  ...
  public function getCountList($values)
  {
    ...
    ->join("ProveedoresTipo","INNER JOIN ProveedoresTipos pt on pt.IdProveedorTipo = Proveedores.IdProveedorTipo")
    ->join("Estados","INNER JOIN Estados e on e.IdEstado = Proveedores.IdEstado")
    ->where("$where")
    ->fetch();
    return $q['cuenta'];
  ...
  function getProveedoresInfo($IdProveedor){
    $ConnectionORM = new ConnectionORM();
    $q = $ConnectionORM->getConnect()->Proveedores
    ->select("*")
