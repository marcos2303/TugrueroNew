...
//    $mLng = ($obj["lngOrigen"]);
//	print_r($obj);die;
    $mGruas = getGruas($link, $obj["latOrigen"], $obj["lngOrigen"], 0.05, $obj["EstadoOrigen"]);
    ...
    if (count($mGruas) > 0) {
        ...
        $mGruas2 = getGruas($link, $obj["latOrigen"], $obj["lngOrigen"], 0.10);
...
//------------------------------
function getGruas($mLink, $mLat, $mLng, $grados,$EstadoOrigen = null) {
    ...
    /*$result = $mLink->query("SELECT idGrua,Token FROM $Tabla WHERE Disponible = '$Disponible' AND ((Latitud BETWEEN '$mTopes[infLat]' AND '$mTopes[supLat]' "
            . "AND Longitud BETWEEN '$mTopes[supLng]' AND '$mTopes[infLng]') OR UPPER(remove_accents(Estado)) = UPPER(remove_accents('$EstadoOrigen')))");*/

	if(strtoupper($EstadoOrigen) == 'MIRANDA' or strtoupper($EstadoOrigen) == 'DISTRITO CAPITAL')
	{
	$result = $mLink->query("SELECT idGrua,Token FROM $Tabla WHERE Disponible = '$Disponible' AND ((Latitud BETWEEN '$mTopes[infLat]' AND '$mTopes[supLat]' "
            . "AND Longitud BETWEEN '$mTopes[supLng]' AND '$mTopes[infLng]') OR UPPER(remove_accents(Estado)) in ('MIRANDA','DISTRITO CAPITAL'))");	
	}else
	{
	$result = $mLink->query("SELECT idGrua,Token FROM $Tabla WHERE Disponible = '$Disponible' AND ((Latitud BETWEEN '$mTopes[infLat]' AND '$mTopes[supLat]' "
            . "AND Longitud BETWEEN '$mTopes[supLng]' AND '$mTopes[infLng]') OR UPPER(remove_accents(Estado)) = UPPER(remove_accents('$EstadoOrigen')))");
	}    
/*echo "SELECT idGrua,Token FROM $Tabla WHERE Disponible = '$Disponible' AND ((Latitud BETWEEN '$mTopes[infLat]' AND '$mTopes[supLat]' "
            . "AND Longitud BETWEEN '$mTopes[supLng]' AND '$mTopes[infLng]') OR UPPER(remove_accents(Estado)) = UPPER(remove_accents('$EstadoDestino')))";die;*/
...
function NuevaSolicitudPUSH($Gruas, $id, $obj) {
    ...
    $timeOpen = gmdate('g:i A - d/m/Y', time() - (4 * 3600));
    $data = array(
